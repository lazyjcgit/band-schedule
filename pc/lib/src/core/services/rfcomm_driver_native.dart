...
class NativeRfcommDriver implements RfcommDriver {
  ...
  static const _method = MethodChannel('bandschedule/classic_spp');
  static const _events = EventChannel('bandschedule/classic_spp/events');
  static const _scanEvents = EventChannel('bandschedule/classic_spp/scan_events');
  ...
  Future<RfcommConnection> connect(
    ...
    _log.info(
      ...
      // 系统里原本有没有配对记录：有就删掉再连。这个值能区分
      // 「旧配对记录挡住了」和「本来就没配对，问题在别处」。
      'bondExisted=${result?['bondExisted']} '
